/*
Project name : Word_Ladder
Created on : Fri Aug 22 15:27:05 2014
...
class Solution {
    ...
    void visit(const string &s , vector<string> p){
        if(s!=last){
	        visited.insert(s);
        }
        q.push(s);
    ...
    void expand(string &s , unordered_set<string> &dict , vector<string> p){
        if(s==last && (len==INT_MAX || p.size()==len)){
            len = p.size();
            soln.push_back(p);
            return;
        }
        int size = adjList[s].size();
        for(int i=0;i<size;++i){
            if(visited.find(adjList[s][i])==visited.end()){
                visit(adjList[s][i] , p);
            }
        }
    }
    void populate_neighbour(const string &s , unordered_set<string> &dict , queue<string> &node_q){
        adjList[s] = deque<string>();
        int size = s.size();
        string temp = s;
        char c;
        
        if(debug)cout<<"populating neighbors for  "<<s<<endl;
        for(int i=0;i<size;++i){
        	c = s[i];
        	if(debug)cout<<"chached char : "<<c<<endl;
        	for(int j=0;j<26;++j)if('a'+j!=c){
        		temp[i] = 'a'+j;
        		if(dict.find(temp)!=dict.end()){
        			adjList[s].push_back(temp);
        			if(debug)cout<<"Adding "<<temp<<endl;
        			if(visited.find(temp)==visited.end()){
        				visited.insert(temp);
        				node_q.push(temp);
        				if(debug)cout<<"adding to queue"<<endl;
        			}
        		}
        	}
        	temp[i] = c;
    ...
    void populate_adj(const string &start , unordered_set<string> &dict){
    	if(debug)cout<<"poppulating ajlist for "<<start<<endl;
        queue<string> node_q;
        node_q.push(start);
        ...
        while(!node_q.empty()){
            ...
            populate_neighbour(node , dict , node_q);
        }
        if(debug)cout<<"================================="<<endl;
    }
public:
    vector<vector<string>> findLadders(string start, string end, unordered_set<string> &dict) {
        dict.insert(end);
        last = end;
        populate_adj(start , dict);
        visited.clear();
        visit(start,vector<string>());
        len = INT_MAX;
        ...
        while(!q.empty()){
            str = q.front();
            p   = path.front();
            if(debug)cout<<"BFS at "<<str<<" with dist "<<p.size()<<endl;
            q.pop();
            path.pop();
...
int main(int argc , char **argv)
{
	...
	while(t--){
		...
		cout<<s.findLadders(str , e , dict).size()<<endl;
	}
	return 0;
}
