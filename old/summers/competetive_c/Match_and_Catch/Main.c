...
void insert(Set *set , char* str , int n , int pos){
	int i;
	...
	node->val = n;
	++node->count;
	node->state = node->state | (1<<pos);
	if(debug && node->count==3)printf(" : val=%d, count=%d ,state=%d\n",node->val,node->count,node->state);
	if(debug)printf("\n");
}
...
int getMin(Node *node){
	...
	int m = node->count==3 && node->state==7 ? node->val : INT_MAX , i , l;
	for(i=0;i<LEN;++i){
...
int main(int argc , char **argv)
{
	...
	if(debug)printf("s1 : \n");
	for(i=0;i<l1;++i){
		for(j=i;j<l1;++j)insert(set,s1+i,j-i+1,0);
	}
	if(debug)printf("s2 :\n");
	for(i=0;i<l2;++i){
		for(j=i;j<l2;++j)insert(set,s2+i,j-i+1,1);
	}
	...
	for(i=1;i<=l1;++i){
		for(j=1;j<=l2;++j){
			if(s1[i-1]==s2[j-1]){
				...
				insert(set,lookup[i][j],strlen(lookup[i][j]),2);
			}
		}
	}
