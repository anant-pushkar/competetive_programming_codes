...
void draw_user(cairo_t *cr , int uid){
	...
	for(i=1;i<window_width/2;++i){
		...
		if(i<window_width/4 && y>20){
			f[nf]=i*freq_scale/594.9+(draw1id==1?100000000:105000000);
...
void button_clicked(GtkWidget *widget,GdkEventButton  *event,gpointer   user_data)
{
	...
	if(event->x>400 && event->x <420 && event->y>550 && event->y<570)quant=quant>2?quant-1:quant;
...
int main(int argc, char *argv[])
{
	...
	for(i=0;i<num_freq;++i){
		...
		f1[i]=rand()%18000+2000;
		f2[i]=rand()%18000+2000;
	}
	gtk_init(&argc, &argv);
