...
$last=end($log["list"]);
if($last["status"]!="end"){
	exit("Session already in progress");
}

$entry = array();
$entry["time"]= date('Y-m-d H:i:s');
$entry["msg"] = "Session started";
...
$bucketlist=array();
$bucketlist["bucket"]=array();
file_get_contents("http://datastore-sit.appspot.com/put?key=cao_bucketlist&value=".json_encode($bucketlist, JSON_FORCE_OBJECT) , false , $context);
